...
def main():
    ...
    id_train_deps_dt_sq_sqrt = load_1d_statistics(in_dist_train_statistics_file).reshape(-1,1)
    print("Fitting KDE to in dist training set")
    kde = KernelDensity(kernel="gaussian", bandwidth=kde_bandwith).fit(id_train_deps_dt_sq_sqrt)
    ...
    id_test_deps_dt_sq_sqrt = load_1d_statistics(in_dist_test_statistics_file).reshape(-1,1)
    ...
    ood_test_deps_dt_sq_sqrt = load_1d_statistics(ood_test_statistics_file).reshape(-1,1)
